...
class client(object):
	def __init__(self, idx, optimu, is_malicious, noise_variance, trainDataSet, assigned_test_dl, learning_rate, net, dev):
		self.idx = idx
		...
		if optimu == 'SGD':
			...
			self.opti = optim.Adam(self.net.parameters(), lr=learning_rate, betas=(0.1, 0.99))
		self.train_dl = None
	...
	def evaluate_model_weights(self, global_parameters):
		...
		with torch.no_grad():
			...
			for data, label in self.test_dl:
				data, label = data.to(self.dev), torch.squeeze(label.to(self.dev))
				preds = torch.squeeze(self.net(data))
				for i in range(data.shape[0]):
					test_loss += torch.sqrt( (label[i, 0] - preds[i, 0]) ** 2 + (label[i, 1] - preds[i, 1]) ** 2 )
...
class ClientsGroup(object):
	...
	def dataSetBalanceAllocation(self):
		...
		for i in range(self.num_of_clients):
			...
			optim = 'SGD' if self.data_set_name in {'mnist-2nn','mnist-cnn'} else 'Adam'
			someone = client(client_idx, optim, is_malicious, self.noise_variance, TensorDataset(torch.tensor(local_train_data), torch.tensor(local_train_label)), test_data_loader, self.learning_rate, self.net, self.dev)
